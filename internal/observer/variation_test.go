...
func TestVariationPersistenceCountsDecodedPolicies(t *testing.T) {
 ...
 if v.CodePolicies!=2 || v.PersistentCodePolicies!=2 || v.CodeCopies!=60 || len(v.Activity)!=3 {t.Fatalf("%+v",v)}
 // A sampled dip invalidates persistence even if the population recovers.
 frames[1].ActiveGenomes[2].Count=4
 v,err=summarizeVariation(frames)
 if err!=nil || v.PersistentCodePolicies!=1 {t.Fatalf("sampled dip ignored: %v %+v",err,v)}
 frames[1].ActiveGenomes[2].Count=5
 // Existing population without copy activity is not an expressed strategy.
 frames[1].Variation[2].Copies=10;frames[1].Copies-=10
 v,err=summarizeVariation(frames)
 if err!=nil || v.PersistentCodePolicies!=1 {t.Fatalf("silent interval ignored: %v %+v",err,v)}
 // A deleted historical record must not masquerade as a new observation.
 frames[1].Variation=frames[1].Variation[:2];frames[1].Copies-=10
 if _,err:=summarizeVariation(frames);err==nil{t.Fatal("missing historical record accepted")}
}
...
func TestVariationRejectsDamagedCountersAndModes(t *testing.T) {
 ...
 for _,mutate:=range []func(*Metrics){
  ...
  next:=base;next.Tick=200;next.Variation=append([]evolution.CopyRecord(nil),base.Variation...);mutate(&next)
  if _,err:=summarizeVariation([]Metrics{base,next});err==nil{t.Fatal("damaged variation accepted")}
 }
}
