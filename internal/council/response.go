...
func checkRound(round, responsePath string, continuation bool) (Checked, error) {
	...
	for _, wb := range r.Worlds {
		...
		if kernel.Hash(w) != wb.SnapshotHash || w.Tick != wb.Tick || w.Config.Seed != wb.Seed || w.Config.MutationPPM != wb.MutationPPM || w.Config.CopyModel != wb.CopyModel || w.Config.Environment != wb.Environment || w.Config.CollectiveAblation != wb.CollectiveAblation || w.Config.Symbols != wb.Symbols || w.Config.BondMotion != wb.BondMotion || w.Config.MetabolicSwitchCost != wb.MetabolicSwitchCost {
