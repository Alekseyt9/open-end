...
Stage 14 can test whether any proposed macro boundary improves prediction under controlled interventions. Boundaries should be discovered on one interval and evaluated on later data, compared with particle-level and matched alternative descriptions. The current inclusion hierarchy and persistence counts do not answer that causal question.
