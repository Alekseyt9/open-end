...
Rolling back an experiment means returning to the frozen source snapshot or control. Switching rules alone does not undo elapsed ticks. The [Stage 8 tree harness](branching.md) preserves and continues several selected cohorts across generations. The [Stage 9 archive](archive.md) adds immutable novelty/Pareto decisions and retention of unusual behavior cells.
