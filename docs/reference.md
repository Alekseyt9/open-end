...
`-ticks` always means **additional** ticks. A snapshot contains configuration, both RNGs, chemical fields, bonds, programs, memory, ancestry, and resource counters. World parameters cannot be overridden with `-load`; they come from the snapshot. The final `state_sha256` line compares complete states. Legacy copying uses snapshot format **2** without DSL state and **3** with it. Encoded copying uses format **4**. Environmental engineering uses format **5**, optionally with encoded copying and/or DSL state. Collective ablations use format **6**; observing groups alone does not change the snapshot format. Rules are **ecology-2**. Stage 1 snapshots (format 1) are rejected as incompatible; there is no implicit migration.
