...
The [switching-cost mechanism](metabolic-switch.md) now tests this hypothesis as an opt-in physical experiment, applied uniformly without assigned cell roles. Its [first 120 trials](../experiments/metabolic-switch/REPORT.md) did not produce complementary specialization. Zero remains the default, and intermediate-product transport timing is the next hypothesis to test. Success still requires useful complementary activity and eventually transmission of organization; larger immobile clusters or fewer metabolic reactions alone do not meet the criterion.
