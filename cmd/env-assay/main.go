// env-assay compares terrain feedback on/off from exactly matched snapshots.
package main
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "env-assay:", err)
		os.Exit(1)
	}
}
...
func run(args []string, out io.Writer) (err error) {
	fs := flag.NewFlagSet("env-assay", flag.ContinueOnError)
	fs.SetOutput(out)
	...
	for _, row := range rows {
		...
		if w.Config.Environment == "" || kernel.Hash(w) != row.StateSHA256 || w.Tick != row.Tick || w.Config.Seed != row.Seed {
	...
	type result struct {
		trial experiment.EnvironmentTrial
		err   error
	}
	...
	for i := 0; i < min(*workers, len(jobs)); i++ {
		...
		go func() {
			defer wg.Done()
			for j := range queue {
				w, frames, r, e := experiment.ContinueEnvironment(j.source, j.mode, *ticks, *every)
				if e == nil {
					stem := fmt.Sprintf("w%03d-%s", j.index/2+1, j.mode)
					r.Case = *condition
