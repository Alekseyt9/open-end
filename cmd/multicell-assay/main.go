// multicell-assay compares fixed and yielding bonds with ancestry diagnostics.
package main
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "multicell-assay:", err)
		os.Exit(1)
	}
}
...
func run(args []string, out io.Writer) (err error) {
	fs := flag.NewFlagSet("multicell-assay", flag.ContinueOnError)
	fs.SetOutput(out)
	...
	for _, row := range rows {
		...
		if w.Config.Environment == "" || w.Config.CollectiveAblation != "" || w.Config.BondMotion != "" || kernel.Hash(w) != row.StateSHA256 || w.Tick != row.Tick || w.Config.Seed != row.Seed {
			return fmt.Errorf("source snapshot metadata mismatch")
		}
		for _, mode := range []string{"intact", "yielding"} {
	...
	type result struct {
		trial experiment.MulticellTrial
		err   error
	}
	...
	for i := 0; i < min(*workers, len(jobs)); i++ {
		...
		go func() {
			defer wg.Done()
			for j := range queue {
				w, frames, r, e := experiment.ContinueMulticell(j.source, j.mode, *ticks, *every, *groupAge)
				if e == nil {
	...
	wg.Wait()
	close(results)
	trials := []experiment.MulticellTrial{}
