// group-assay compares collective ablations from exactly matched snapshots.
package main
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "group-assay:", err)
		os.Exit(1)
	}
}
...
func run(args []string, out io.Writer) (err error) {
	fs := flag.NewFlagSet("group-assay", flag.ContinueOnError)
	fs.SetOutput(out)
	...
	for _, row := range rows {
		...
		if w.Config.Environment == "" || w.Config.CollectiveAblation != "" || kernel.Hash(w) != row.StateSHA256 || w.Tick != row.Tick || w.Config.Seed != row.Seed {
	...
	for i := 0; i < min(*workers, len(jobs)); i++ {
		...
		go func() {
			defer wg.Done()
			for j := range queue {
				w, frames, r, e := experiment.ContinueCollectives(j.source, j.mode, *ticks, *every, *groupAge)
				if e == nil {
