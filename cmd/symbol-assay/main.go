// symbol-assay compares persistent, scrambled, and unreadable reception from matched snapshots.
package main
...
func main() {
	if err := run(os.Args[1:], os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "symbol-assay:", err)
		os.Exit(1)
	}
}
...
func run(args []string, out io.Writer) (err error) {
	fs := flag.NewFlagSet("symbol-assay", flag.ContinueOnError)
	fs.SetOutput(out)
	input := fs.String("input", "", "completed experiment directory")
	dest := fs.String("out", "", "new output directory")
	condition := fs.String("case", "symbols", "source case to compare")
	ticks := fs.Int("ticks", 10000, "additional ticks in each arm")
	...
	if *input == "" || *dest == "" || *ticks < 1 || *every < 1 || *every > *ticks || *workers < 1 || *workers > 256 || fs.NArg() != 0 {
	...
	for _, row := range rows {
		...
		if w.Config.Symbols != "persistent" || kernel.Hash(w) != row.StateSHA256 || w.Tick != row.Tick || w.Config.Seed != row.Seed {
	...
	start := time.Now()
	status := map[string]any{"status": "running", "input": *input, "case": *condition, "workers": *workers, "ticks": *ticks, "every": *every, "total": len(jobs), "started_utc": start.UTC().Format(time.RFC3339)}
	...
	for i := 0; i < min(*workers, len(jobs)); i++ {
		...
		go func() {
			defer wg.Done()
			for j := range queue {
				w, frames, r, e := experiment.ContinueSymbols(j.source, j.mode, *ticks, *every)
				if e == nil {
					stem := fmt.Sprintf("w%03d-%s", j.index/3+1, j.mode)
					r.Case = *condition
