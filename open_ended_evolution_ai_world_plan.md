...
The [cell contribution assay](experiments/multicell-roles/REPORT.md) tests preserved mixed-genome groups with COPY-lineage interventions. Its first 116 continuations show offspring provisioning, no TRANSFER between original cell lineages, and no targeted signal use. The [mobility and chemistry controls](experiments/multicell-mechanisms/REPORT.md) then show 14/14 anchored pairs agreeing outside bond fields, while a proportional tracer attributes 1.31% of tagged Y consumption to other selected lineages. All observed original cells still execute both reactions.

The optional [switching-cost experiment](experiments/metabolic-switch/REPORT.md) adds persistent physiological preparation and tests it in 64 population continuations plus 56 chemical witness runs. It did not yield complementary roles at the measured windows, and nonzero costs reduced peer chemical contribution. Zero remains the default. Next, test intermediate-product transport timing and resource access with matched controls, before adding stronger penalties or treating additional copying as organizational progress. Inheritance of useful organization across group generations remains the target.
